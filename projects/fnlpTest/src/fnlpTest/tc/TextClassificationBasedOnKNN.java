...
public class TextClassificationBasedOnKNN {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Knn Precision:" + ((float) knnCount / testset.size()) + "(" + knnCount + "/"
                + testset.size() + ")");
        ...
        for (int test = 0; test < percents_cs.length; test++) {
            ...
            time_times++;
            System.out.println("Knn Precision(" + percents_cs[test] + "):" + ((float) count / testset.size()) + "("
                    + count + "/" + testset.size() + ")" + "  " + (time_ed - time_st) + "ms");
        }
        ...
        for (int test = 0; test < percents_csmax.length; test++) {
            ...
            time_sum += time_ed - time_st;
            time_times++;
            System.out.println("Knn Precision(" + percents_csmax[test] + "):" + ((float) count / testset.size()) + "("
                    + count + "/" + testset.size() + ")" + "  " + (time_ed - time_st) + "ms");
        }
        knn.noFeatureSelection();
        ...
        for (int test = 0; test < percents_ig.length; test++) {
            ...
            time_sum += time_ed - time_st;
            time_times++;
            System.out.println("Knn Precision(" + percents_ig[test] + "):" + ((float) count / testset.size()) + "("
                    + count + "/" + testset.size() + ")" + "  " + (time_ed - time_st) + "ms");
        }

        System.out.println("..Testing Bayes complete!");
        for (int i = 0; i < percents_cs.length; i++) {
            System.out.println("Knn Precision CS(" + percents_cs[i] + "):" + ((float) counts_cs[i] / testset.size())
                    + "(" + counts_cs[i] + "/" + testset.size() + ")");
        }
        ...
        for (int i = 0; i < percents_ig.length; i++) {
            System.out.println("Knn Precision IG(" + percents_ig[i] + "):" + ((float) counts_ig[i] / testset.size())
                    + "(" + counts_ig[i] + "/" + testset.size() + ")");
        }

        System.out.println("\nTrain time: " + time_train + "(ms) for " + trainset.size() + " train instances\n");
        if (time_times > 0) {
            System.out.println("Ave Test time: " + time_sum / time_times + "(ms) for " + testset.size()
                    + " test instances\n");
        }
    }
