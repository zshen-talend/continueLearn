...
public class ToManyExample {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 2 || !"-h".equals(args[0])) {
            System.out.println("Usage: java " + ToManyExample.class.getName() + " -h ENV_HOME");
            System.exit(1);
        }
    ...
    private ToManyExample(String homeDir) throws DatabaseException {
        ...
        try {
            ...
            /*
             * A standard secondary config; duplicates, key creators and key nullifiers are specified below.
             */
            ...
            /*
             * Open the secondary database for personByEmail. This is a one-to-many index because duplicates are not
             * configured.
             */
            ...
            /*
             * Open the secondary database for personByAnimal. This is a many-to-many index because duplicates are
             * configured. Foreign key constraints are specified to ensure that all animal keys exist in the animal
             * database.
             */
            ...
            personByAnimal = env.openSecondaryDatabase(txn, "personByAnimal", personDb, secConfig);
    ...
    /**
     * Adds, updates, prints and deletes Person records with many-to-many and one-to-many secondary indices.
     */
    private void exec() throws DatabaseException {
     ...
     */
    private Animal insertAndPrintAnimal(String name, boolean furry) throws DatabaseException {
        ...
        if (status == OperationStatus.SUCCESS) {
            ...
            System.out.println("Animal was not inserted: " + name + " (" + status + ')');
        }
     ...
     */
    private boolean deleteAnimal(String name) throws DatabaseException {
     ...
     */
    private void printByIndex(SecondaryDatabase secDb) throws DatabaseException {
        ...
        try {
            while (cursor.getNext(secKey, priKey, priData, null) == OperationStatus.SUCCESS) {
                ...
                System.out.println("Index key [" + keyBinding.entryToObject(secKey) + "] maps to primary key [" + person.name
                        + ']');
            }
        } finally {
            cursor.close();
     ...
     */
    private void putPerson(Person person) throws DatabaseException {
     ...
     */
    private boolean deletePerson(String name) throws DatabaseException {
    ...
    /**
     * Returns the set of email addresses for a person. This is an example of a multi-key creator for a to-many index.
     */
    private class EmailKeyCreator implements SecondaryMultiKeyCreator {

        public void createSecondaryKeys(SecondaryDatabase secondary, DatabaseEntry primaryKey, DatabaseEntry primaryData,
    ...
    /**
     * Returns the set of favorite animals for a person. This is an example of a multi-key creator for a to-many index.
     */
    ...
    /**
     * A utility method to copy a set of keys (Strings) into a set of DatabaseEntry objects.
     */
    private void copyKeysToEntries(Set<String> keys, Set<DatabaseEntry> entries) {

        for (String string : keys) {
            DatabaseEntry entry = new DatabaseEntry();
            keyBinding.objectToEntry(string, entry);
            entries.add(entry);
        }
    }

    /**
     * Removes a given key from the set of favorite animals for a person. This is an example of a nullifier for a
     * to-many index. The nullifier is called when an animal record is deleted because we configured this secondary with
     * ForeignKeyDeleteAction.NULLIFY.
     */
    private class AnimalKeyNullifier implements ForeignMultiKeyNullifier {

        public boolean nullifyForeignKey(SecondaryDatabase secondary, DatabaseEntry primaryKey, DatabaseEntry primaryData,
