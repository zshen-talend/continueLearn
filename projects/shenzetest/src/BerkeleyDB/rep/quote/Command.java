...
enum Command {
    ...
    /*
     * Indicates whether the command is manifest, that is its the enum name itself.
     */
    ...
    static Command getCommand(String line) throws InvalidCommandException {
        ...
        for (Command c : Command.values()) {
            if (c.manifest && c.name().equalsIgnoreCase(command)) {
                ...
                /* Extra token. */
                throw new InvalidCommandException("Unexpected argument: " + tokenizer.nextToken() + " for command: " + command);
            }
        }
