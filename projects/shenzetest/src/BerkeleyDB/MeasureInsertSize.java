...
/**
 * MeasureInsertSize inserts a given set of key/value pairs in order to measure the disk space consumed by a given data
 * set.
 * 
 * To see how much disk space is consumed, simply add up the size of the log files or for a rough estimate multiply the
 * number of files by 10 MB.
 * 
 * This program does sequential inserts. For random inserts, more disk space will be consumed, especially if the entire
 * data set does not fit in the cache.
 * 
 * This program does not insert into secondary databases, but can be used to measure the size of a secondary by
 * specifying the key and data sizes of the secondary records. The data size for a secondary record should be specified
 * as the size of the primary key.
 * 
 * Checkpoints are performed by this program as usual, and checkpoints will added to the size of the log. This is
 * realistic for a typical application but note that a smaller disk size can be obtained using a bulk load.
 * 
...
public class MeasureInsertSize {
    ...
    public void doInserts() throws DatabaseException {
